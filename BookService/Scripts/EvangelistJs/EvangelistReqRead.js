            ...
            { headerText: "Country", rowText: "Country" },
            {
                headerText: "Edit", rowText: {
                    action: function (item) {
                        return function () {
                            window.location.href = '/EvangelistRequests/Edit?id=' + item.RequestId;
                        }
                    }
                }
            },
                    ...
                    action: function (item) {
                        return function () {
                            if (item.Status == 0) {
                                ...
                                try {
                                    $.ajax({
                                        url: '/ApproveEvangelistRequest?id=' + item.RequestId,
                                        type: 'GET',
                    ...
                    action: function (item) {
                        return function () {
                            if (item.Status == 0) {
                                ...
                                try {
                                    $.ajax({
                                        url: '/RejectEvangelistRequest?id=' + item.RequestId,
                                        type: 'GET',
                                        contentType: 'application/json',
                                        headers: {
                                            "Content-Type": "application/x-www-form-urlencoded",
